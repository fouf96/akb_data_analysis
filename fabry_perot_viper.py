...
def load_data_set(path):
    ...
    delays = np.load(os.path.join(path, "ir_delay_file_" + filename +".npy")) 
...
def average_transmission_with_counts(data: ndarray, counts: ndarray):
    ...
    signal = absorption.take(1, axis=-1).take(1, axis=-1) - absorption.take(0, axis=-1).take(1, axis=-1) - absorption.take(1, axis=-1).take(0, axis=-1) + absorption.take(0, axis=-1).take(0, axis=-1)
    return signal
...
def average_transmission_with_weights(data: ndarray, weights: ndarray):
    ...
    # Average data
    avg_data = np.average(scatter_free_transmission, axis=0, weights=weights)
    # Calculate absorption
    absorption = -np.log10(avg_data)
    # Calculate difference signal
    signal = absorption.take(1, axis=-1).take(1, axis=-1) - absorption.take(0, axis=-1).take(1, axis=-1) - absorption.take(1, axis=-1).take(0, axis=-1) + absorption.take(0, axis=-1).take(0, axis=-1)
...
def average_signal_with_s2s(data: ndarray, s2s_std: ndarray):
    ...
    # Calculate absorption
    absorption = -np.log10(scatter_free_transmission)
    # Calculate difference signal
    signal = absorption.take(1, axis=-1).take(1, axis=-1) - absorption.take(0, axis=-1).take(1, axis=-1) - absorption.take(1, axis=-1).take(0, axis=-1) + absorption.take(0, axis=-1).take(0, axis=-1)
    ...
    # to use as weights
    weights = np.float_power(s2s_std.sum(axis=3), -2)
...
if __name__ == "__main__":
    path = r"C:\Users\H-Lab\Documents\data_analysis\20200917_ITx_FPVIPER_184_000"
...
#     print(t1.shape)
#     print(t4.shape)
    old_format_t1 = generate_legacy_data_format(t1, delays[:,0], probe_axis, pump_pixels[:,0].astype(int))
    
    old_format_t3 = generate_legacy_data_format(t3, delays[:,0], probe_axis, pump_pixels[:,0].astype(int))
    print(np.array_equal(t1[:, 2, :].T, old_format_t1[66:98, 1:]))
    # print(t1[:, 2, :].T.shape, old_format[33:65, 1:].shape)
    np.savetxt(os.path.join(path, "old_format_t1.txt"), old_format_t1, delimiter="\t")
    np.savetxt(os.path.join(path, "old_format_t3.txt"), old_format_t3, delimiter="\t")
